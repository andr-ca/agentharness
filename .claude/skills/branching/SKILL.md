...
A consumer project without full harness-hook wiring has no automatic
signal for this rule — `tools/harness-session-start.sh` is an optional,
standalone script (no hook installation required) that prints this same
warning at session start when the tree is dirty; wire it into a
project's own session-start step if useful (issue #249, item 7).
