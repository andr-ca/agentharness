...
| AC-22 | GitHub settings apply minimally and read back, or bootstrap stays resumably incomplete. | 5 | `remote/github/protection.py` | `tests/integration/test_github_reconcile.py` | Before/plan/after sanitized API snapshots | implemented |
...
| AC-26 | Decommission uses protected three-PR transaction and leaves no orphan required context. | 5 | `remote/github/decommission.py` | state-machine integration and sandbox E2E | Three PRs plus final protection read-back | implemented |
