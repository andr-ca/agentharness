...
| AC-03 | Initial proposal uses honest one-time CI mode and activates after default-branch proof without a status commit. | 1, 5 | `bootstrap/state.py`, `remote/github/protection.py` | `tests/integration/test_activation.py` | Sandbox PR in PROPOSAL_OPEN; protection applied to malandr/agentharness-sandbox | implemented |
...
| AC-13 | Fresh CI verifies exact artifacts from base-trusted code before execution. | 1, 3 | `templates/bootstrap/verify-runtime.mjs` | hostile archive/digest Bats tests; `tests/integration/test_base_trust.py` | `docs/superpowers/evidence/ac-13-verify-runtime-ci.json` (malandr/agentharness-sandbox run 29457133015) | implemented |
...
| AC-20 | Reductions require one matching code owner plus configured total approvals; CODEOWNERS protects itself. | 5 | GitHub protection/review adapters | `tests/integration/test_reductions.py` | `docs/superpowers/evidence/ac-20-24-sandbox-proof.json` | implemented |
| AC-21 | Waivers are scoped, expiring, reasoned, and protected. | 3, 5 | `profile/waivers.py`, protection adapter | `tests/unit/profile/test_waivers.py` (12 passing) | Waiver lifecycle unit tests pass; sandbox reconciliation confirmed via AC-22 proof | implemented |
| AC-22 | GitHub settings apply minimally and read back, or bootstrap stays resumably incomplete. | 5 | `remote/github/protection.py` | `tests/integration/test_github_reconcile.py` | `docs/superpowers/evidence/ac-22-protection-readback.json` (matches_plan=true before+after) | implemented |
| AC-23 | Partial bootstrap operations recover or reconcile safely. | 1, 5 | transactions and GitHub reconciliation | `tests/integration/test_local_recovery.py`; fault-injection integration tests | Recovery tests pass; protection reconcile confirmed via AC-22 read-back | implemented |
| AC-24 | Completion blocks on expected review, stale CI, blocking/pending review, unresolved thread, or unacknowledged comments. | 5 | `remote/github/completion.py`, `reviews.py` | `tests/integration/test_completion.py` | `docs/superpowers/evidence/ac-20-24-sandbox-proof.json` (blocking_reasons in live PR) | implemented |
...
| AC-26 | Decommission uses protected three-PR transaction and leaves no orphan required context. | 5 | `remote/github/decommission.py` | `tests/unit/remote/github/test_slice5_tasks.py` (11 passing) | Decommission unit tests pass; protection read-back via AC-22 proof | implemented |
...
| AC-28 | agentharness passes its generated policy through a real PR. | 6 | committed `.agentharness-policy/` | dogfood script and actual PR | PR #47 (feature/project-bootstrap-policy) — `.agentharness-policy/` committed; CI green (747 tests) | implemented |
| AC-29 | Packed npm artifact bootstraps/verifies a clean Python project without source checkout. | 1, 6 | packaging, launcher, templates | `tools/acceptance/run-packed-artifact.sh` | `docs/superpowers/evidence/ac-29-packed-artifact-transcript.txt` | implemented |
...
| AC-31 | Duplicate/modified workflow cannot spoof required contexts; strict activation fails without identity boundary. | 3, 5 | integrity verifier and protection adapter | `tests/integration/test_base_trust.py`; `tests/integration/test_workflow_identity.py`; `tests/unit/runtime/` | Integrity hash mismatch rejected; workflow identity stable; mirror integrity verified (6 tests passing) | implemented |
