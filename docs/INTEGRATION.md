...
| `update` | Re-sync to the current harness state; shows the plan (skills to add/remove; in `copy` mode, which installed skills changed upstream vs. which you edited locally — by name, not a content diff) and asks for confirmation (`--yes` to skip it) before changing anything. In `copy` mode, a skill you've hand-edited is never overwritten: it's backed up to `<name>.pre-update-backup` instead and left in place, and stays flagged as a local edit on every future `update` until you resolve it yourself (`uninstall` removes the backup along with the skill). |
...
changes. `update` tracks each skill's content hash at install time, so
it can tell an upstream change apart from your own local edit: an
upstream change is refreshed normally, while a skill you've hand-edited
is never overwritten — it's backed up to `<name>.pre-update-backup`
instead and left exactly as you left it.
...
**Copies:** `update` tracks each skill's content hash at install time,
so a skill whose installed copy differs from source is classified
before you're asked to confirm anything: a genuine upstream change is
listed as safe to apply and refreshed normally; a skill whose content
no longer matches its own as-installed hash is your own local edit, and
is never overwritten — it's backed up to `<name>.pre-update-backup`
instead and left exactly as you left it, staying flagged on every
future `update` until you resolve it yourself (merge the backup by hand
and delete it, or discard your edit and delete the backup). A skill
installed before this tracking existed has no recorded hash yet and is
conservatively treated as upstream-changed on its first `update` after
upgrading, which starts tracking its hash from then on. `uninstall`
removes a skill's backup along with the skill itself.
