...
triggered by `workflow_run: types: [completed]` that reruns
issue-analysis's failed/timed-out jobs via `gh run rerun --failed`, up
to 2 retries (bounded by `github.event.workflow_run.run_attempt < 3`,
