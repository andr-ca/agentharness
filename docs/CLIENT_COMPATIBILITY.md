...
| Cursor | `.cursorrules` (legacy, always-on) or `.cursor/rules/*.mdc` (current; per-file `alwaysApply`/`globs`/`description` activation) | `.cursorrules` loads unconditionally; `.mdc` files load per their own frontmatter | ✅ `.cursor/rules/agentharness-router.mdc` + one `.mdc` per skill, generated by `tools/generate-cursor-rules.sh` — **live-verified 2026-08-19** (`cursor-agent --print`: the router's completion-gate and lock commands answered correctly with no other rule installed) |
