...
**Correction (2026-07-13, same day):** this table originally classified
GitHub Copilot as persona-only, based on an earlier research pass that
apparently found the *cloud coding agent's* custom-agent feature (a
different Copilot surface) without finding the CLI/VS Code custom-agents
mechanism's own docs
(<https://docs.github.com/en/copilot/how-tos/copilot-cli/customize-copilot/create-custom-agents-for-cli>,
<https://docs.github.com/en/copilot/reference/custom-agents-configuration>),
which confirm genuine sub-agent delegation with an isolated context
window. Flagged and fixed the same day, not silently corrected — this is
exactly the kind of research-accuracy failure this document's "not
verified against a live session" caveat exists to catch when it happens
in the other direction.

**Practical read:** six tools (Claude Code, Codex CLI, OpenCode, Cursor,
Kilo Code, GitHub Copilot) support genuine task delegation to a separate
specialized agent — structurally close enough that this repo now ports
its one real subagent (`coding-guidelines-reviewer`, a read-only
reviewer scoped to `.github/CODING_GUIDELINES.md`'s rigor tiers) across
all six via `tools/generate-{codex,opencode,cursor,kilo,copilot}-agents.sh`,
the same relationship `.claude/skills/` has to `.agents/skills/`. Gemini
