...
| Rigor-tier profiles | `patterns/profiles/README.md` | guide | Selecting a tier via `.agentharness-profile`, precedence order, current enforcement state (Python, Go, and JS/TS `node --test`/Vitest/Jest via `enforce-profile`; opt-in on push via `--with-coverage-hook`; advisory for other project types/runners) |
...
| Claude Code statusline | `tools/statusline.sh` | script | Reads Claude Code's session JSON on stdin, prints model/dir/branch/context% plus harness state (publish-authority mode, agent-lock status). Installed opt-in via `harness-link.sh init --with-statusline` — see docs/CLIENT_COMPATIBILITY.md's Statusline section |
...
| Session-start worktree reminder | `tools/harness-session-start.sh` | script | Optional, standalone (no hook wiring): warns — never blocks — when the working tree is dirty and not already inside a git worktree, printing the exact `git worktree add` command; call from a consumer's AGENTS.md 'start of every session' step (issue #249, item 7) |
...
| Skill-symlink verifier | `tools/verify-skill-symlinks.sh` | script | Verifies `.agents/skills/` and `.qwen/skills/` both stay 1:1 with `.claude/skills/` — every Agent-Skills-standard tool (Codex, Copilot, Gemini, ...) reads the former, Qwen Code reads the latter, so drift silently hides a skill from them while Claude still sees it |
...
| Project bootstrap (first-run discovery, interview, and apply) | `src/agentharness/bootstrap/` | package | First run in a project — `agentharness bootstrap plan` inventories what the repo already configures and lists the decisions the owner must make; `apply --confirm <hash>` creates only what was answered for. Read-only until confirmed; never overwrites existing config |
| Project bootstrap skill | `.claude/skills/project-bootstrap/SKILL.md` | skill | Conducting the first-run interview — turns `bootstrap plan` output into a one-question-at-a-time conversation with the owner, separating verified detections from recommendations |
| Policy core (⚠ EXPERIMENTAL — gates, profiles, plugins not wired into public CLI) | `src/agentharness/` | package | Deterministic Python policy core. The bootstrap surface above is released; the remaining gates/profiles/plugins machinery is experimental and the public npm launcher does not invoke it. See docs/STATUS.md for the version split |
...
| QWEN.md generator | `tools/generate-qwen-md.sh` | script | Builds Qwen Code's routing-only `QWEN.md` (its default context filename) from `CLAUDE.md` plus a name+description skill index pointing at `.qwen/skills/` — Qwen Code's own skill-discovery directory, not `.agents/skills/` (verified against the CLI's own bundled docs; see the generator's header) |
...
| Qwen Code custom-agent generator | `tools/generate-qwen-agents.sh` | script | Ports `.claude/agents/*.md` subagent definitions to Qwen Code's own custom-subagent format (`.qwen/agents/*.md`) — genuine isolated-context sub-agent delegation, distinct from `generate-qwen-md.sh`'s routing/skill mechanism; `tools`/`disallowedTools`/`approvalMode` are NOT ported (Qwen's tool-name vocabulary differs from Claude Code's, so a verbatim `tools:` copy would leave the subagent with an empty toolset) |
...
| Qwen Code routing rules (generated; skills load on demand from `.qwen/skills/`) | `QWEN.md` | doc (generated by `tools/generate-qwen-md.sh`; do not hand-edit) |
...
| Qwen Code custom agents (generated; ported from .claude/agents/, distinct from QWEN.md) | `.qwen/agents/` | doc (generated by `tools/generate-qwen-agents.sh`; do not hand-edit) |
...
| Context registry (Context Plane Slice 1 — governed/versioned context; schema validated in tools/verify-content-quality.py) | `context.yaml` | doc |
...
| Project bootstrap and deterministic policy design (first-run surface implemented; gates/profiles still experimental) | `docs/superpowers/specs/2026-07-14-project-bootstrap-policy-design.md` | design specification |
| Project bootstrap implementation master plan (first-run surface delivered; later slices open) | `docs/superpowers/plans/2026-07-14-project-bootstrap-policy-master-plan.md` | implementation plan |
