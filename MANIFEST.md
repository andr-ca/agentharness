...
| pre-merge-commit dispatcher | `.github/hooks/pre-merge-commit` | git hook | Required to block merge commits to trunk branches — `core.hooksPath` only invokes a file literally named `pre-merge-commit`; git only falls back to `pre-commit` for merge commits under certain conditions, and when that fallback fails, merge commits bypass protection entirely (see issue |
